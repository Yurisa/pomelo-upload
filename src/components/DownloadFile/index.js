...
class DownloadFile extends Component {
  ...
  MathSpeend(item) {
    ...
    return (
      this.FileSize(speed) +
      '/s  剩余时间:' +
      this.formatSeconds(remaining_time)
    );
  }
    ...
    return file ? (
      <div className='task-item'>
        ...
        <div className='task-actions'>
          <Icon
            type={this.ControlButton(file && file.state)}
            onClick={(e) => this.ControlTrans(e, file, index)}
          />
          <Icon
            type='close'
            onClick={(e) => this.ControlTrans(e, file, index)}
        ...
        <div className='task-speed'>
          ...
          {file.state === 'progressing' ? (
            <span>{this.MathSpeend(file)}</span>
          ) : null}
        </div>
      </div>
    ) : null;
