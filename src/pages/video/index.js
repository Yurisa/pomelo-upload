...
class Video extends Component {
	...
	componentWillMount() {
		ipcRenderer.on('win-data', (event, data)=>{//接收视频文件列表的数据
      setTimeout(()=>{
	...
	VideoCache = () => {
		...
		try {
			this.setState({
				CacheWidth: (media.buffered.end(media.buffered.length - 1) / media.duration).toFixed(2) * 100 + '%'
			});
		} catch (e) {
		}
	}
  ...
  handleMouseDown = (e) => {
	...
	render() {
		const { VideoHeight, NowPlay, VolumnState, PlayButtonState, animation, BarAnimation, ProcessWidth, CacheWidth, TimeText, FullButton } = this.state;
		return (
			<div className="cd-video-player-main" ref="VideoPlayer" onMouseDown={() => { this.setState({ VolumnState: false }) }} tabIndex="-1">
				<div className="cd-video-main">
					...
					<div className={`cd-video-fliter ${animation}`} onClick={() => this.VideoPlayerCommend('play')}>
						{
							PlayButtonState === 'pause' ? <Icon type="caret-right" /> : PlayButtonState === 'play' ? <Icon type="pause" /> : <Icon type="loading"/>
						}
					</div>
            <div className={`cd-video-control ${BarAnimation}`} onMouseOver={this.ShowControl} onMouseOut={this.HideControl}>
                <div className={`cd-video-play ${PlayButtonState}`} onClick={() => this.VideoPlayerCommend('play')}></div>
                <div className="cd-video-player-slider-container"  onMouseDown={(e) => this.TimeChange(e)} ref="slider">
                    <div className="cd-player-process-bar" style={{width:ProcessWidth}}>
                        <span></span>
                    </div>
                    <div className="VideoTempBar" style={{width:CacheWidth}}></div>
                </div>
                <div className="cd-video-player-time">{TimeText}</div>
                <div className={`${(VolumnState ? 'cd-video-player-volumn-active':'')}`} style={{float: 'left', marginTop: '5px'}} onMouseDown={(e) => this.handleMouseDown(e)}>
									<Icon type='sound' />
								</div>
                <div className="FullButton" onClick={() => this.FullScreen(FullButton)}></div>
								{
									VolumnState ? (
