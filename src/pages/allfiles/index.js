...
class AllFiles extends Component {
  ...
  selectFiles = (event, item, index) => {
    ...
    if (event.button === 0) {
      ...
      } else if (DiskData.KeyFlag === 'Shift') {
        //Shift多选
        let Start = index,
          End;
    ...
    switch (command) {
      case 'download':
        ...
        downloadFiles.forEach((file) => {
          electron.remote
            .getCurrentWindow()
            .webContents.downloadURL(file.downloadUrl);
        });
        break;
      case 'open':
        this.openFile();
  ...
  renderDownloadingab = () => {
    ...
    return (
      <div>
        正在下载 {len ? <span className='downloadListNum'>{len}</span> : null}
      </div>
    );
  };
    ...
    if (!item) {
      ...
      if (fileList.length > 0) {
        if (fileList.every((item) => item.file_suffix === 'mp3')) {
          ...
          fileList.every((item) => item.file_suffix === 'mp3' || 'avi')
        ) {
          fileList.forEach((item) => {
            item.active = false;
          });
          ...
          <div ref='main'>
            {files.length > 0 &&
              files.map((item, index) => {
                ...
                return (
                  <div
                    className={`cd-disk-block-file ${
                      item.active ? 'cd-disk-block-file-active' : null
                    }`}
            ...
            TransformData.map((file, index) => {
              return (
                <DownloadFile
                  ...
                  key={file}
                  index={index}
                  ControlTrans={this.ControlTrans}
                />
              );
            })
          ) : (
